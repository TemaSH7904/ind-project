...
# --- ИЗМЕНЕНИЕ ДЛЯ СЛОЖНОГО УРОВНЯ ---
# Принимаем IP вторым аргументом. Если пусто — используем localhost.
SERVER_IP="${2:-localhost}"

# Конфигурация
API_HOST="http://${SERVER_IP}:8080"
DB_HOST="${SERVER_IP}"
DB_PORT="5432"
...
# --- ИЗМЕНЕНИЕ: Проверка аргументов разрешает 1 или 2 параметра ---
if [ $# -lt 1 ] || ! [[ $1 =~ ^[1-3]$ ]]; then
    echo "Использование: $0 <уровень_проверки> [IP_адрес]"
